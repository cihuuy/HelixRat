...
#define pbox1(a, b, c)                                                       \
    ({                                                                       \
        ...
        uint32_t s1 = ROTLEFT(e, 6) ^ ROTRIGHT(e, 9) ^ ROTLEFT(e, e % 32);   \
        s0 *e + s1;                                                          \
    })
...
#define pbox2(a, b, c)                                                                \
    ({                                                                                \
        ...
        uint32_t s1 = ROTRIGHT(e, s0 % 32) ^ ROTLEFT(e, 2) ^ ROTRIGHT(e, e % 32);     \
        s0 + e *s1;                                                                   \
    })


static inline std::string helixrat_hash(uint8_t *data, uint64_t length)
{
...
int main(int argc, char **argv)
{
    ...
    // std::cout << helixrat_check((uint8_t *)argv[1], strlen(argv[1])) << std::endl;

    uint64_t counter = 1;
    while (counter < (15000000000 / 32))
    {
        std::cout << helixrat_hash((uint8_t *)&counter, sizeof(counter));
        counter++;
    }
    return 0;
}